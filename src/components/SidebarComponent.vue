<template>
    ...
    <!-- <div id="bg-overlay" class="bg-black fixed w-full min-h-screen bg-opacity-50 hidden"></div>
    <aside id="sidebarMobile" class="bg-white w-2/3 h-screen hidden lg:hidden border-r fixed right-0 flex-col justify-between ">
        <div>
            <div class="h-20 flex items-center gap-4 px-4 border-b">
                <button class="text-2xl"><i class="fa-solid fa-circle-xmark"></i></button>
            ...
            <div class="px-4 py-6">
                ...
                <ul class="mt-2 flex flex-col gap-3">
                    <li class="w-full flex" v-for="main in mains" :key="main.name">
                        <router-link :to="main.url" :class="['w-full py-2 text-slate-600 px-2 rounded-lg hover:bg-slate-100 hover:duration-150', isActive(main.name) ? 'bg-slate-800 text-white hover:bg-slate-700 hover:duration-150' : '']"><i :class="main.icon + ' w-10'"></i> {{ main.name }}</router-link>
                    </li>
                </ul>
                <h5 class="text-xl text-slate-500 font-medium mt-6">Dashboard</h5>
                <ul class="mt-2 flex flex-col gap-3">
                    <li class="w-full flex" v-for="dashboard in dashboards" :key="dashboard.name">
                        <router-link :to="dashboard.url" class="w-full py-2 text-slate-600 px-2 rounded-lg hover:bg-slate-100 hover:duration-150"><i :class="dashboard.icon + ' w-10'"></i> {{ dashboard.name }}</router-link>
                    </li>
                </ul>
                <h5 class="text-xl text-slate-500 font-medium mt-6">Pengaturan</h5>
                <ul class="mt-2 flex flex-col gap-3">
                    <li class="w-full flex" v-for="setting in settings" :key="setting.name">
                        <router-link :to="setting.url" class="w-full py-2 text-slate-600 px-2 rounded-lg hover:bg-slate-100 hover:duration-150"><i :class="setting.icon + ' w-10'"></i> {{ setting.name }}</router-link>
                    </li>
                </ul>
            </div>
        </div>
